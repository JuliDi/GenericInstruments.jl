...
## 
# Reset
reset_instr(obj::T) where {T <: Union{DG1022Z}} = write(obj, "*RST")
# Clear
clear_instr(obj::T) where {T <: Union{DG1022Z}} = write(obj, "*CLS")
# IDN
get_idn(obj::T) where {T <: Union{DG1022Z}} = query(obj, "*IDN?")
...
# Offset
set_offs(obj::T;ch="1",volt=0.01) where {T <: Union{DG1022Z}} = write(obj, "SOURce$ch:VOLT:OFFset $volt")
# High Level
set_hilev(obj::T;ch="1",volt=0.01) where {T <: Union{DG1022Z}} = write(obj, "SOURce$ch:VOLT:HIGH $volt")
# Low Level
set_lolev(obj::T;ch="1",volt=0.01) where {T <: Union{DG1022Z}} = write(obj, "SOURce$ch:VOLT:LOW $volt")
# Frequency
set_freq(obj::T;ch="1",fr=1000) where {T <: Union{DG1022Z}} = write(obj, "SOURce$ch:FREQ $fr")
# Period
# set_per(obj::T;ch="1",func="sin",per=0.001) where {T<:Union{DG1022Z}} = write(obj,"SOURce$ch:FUNC:$func:PER $per") # Rigol_DG1022Z allows for setting period per channel
set_per(obj::T;ch="1",per=0.001) where {T <: Union{DG1022Z}} = write(obj, "SOURce$ch:PER $per")
# Phase
set_phase(obj::T;ch="1",phase=90) where {T <: Union{DG1022Z}} = write(obj, "SOURce$ch:PHAS $phase") # Rigol_DG1022Z does not allow for setting an angle unit
# Symmetry
set_sym(obj::T;ch="1",func="ramp",sym=50) where {T <: Union{DG1022Z}} = write(obj, "SOURce$ch:FUNC:$func:SYMM $sym")
# Both Edge times
set_both_edg(obj::T;ch="1",func="pulse",dt=10e-9) where {T <: Union{DG1022Z}} = write(obj, "SOURce$ch:FUNC:$func:TRAN $dt")
# Lead Edge time
set_lead_edg(obj::T;ch="1",func="pulse",dt=10e-9) where {T <: Union{DG1022Z}} = write(obj, "SOURce$ch:FUNC:$func:TRAN:LEAD $dt")
# Trail Edge time
set_trail_edg(obj::T;ch="1",func="pulse",dt=10e-9) where {T <: Union{DG1022Z}} = write(obj, "SOURce$ch:FUNC:$func:TRAN:TRA $dt")
# Width
set_width(obj::T;ch="1",func="pulse",wd=1e-4) where {T <: Union{DG1022Z}} = write(obj, "SOURce$ch:FUNC:$func:WIDTh $wd")
#
## OUTPUT
# Output Load
set_load(obj::T;ch="1",load="INF") where {T<:Union{DG1022Z}} = write(obj,"OUTP$ch:LOAD $load")
# Output Polarity
set_pol(obj::T;ch="1",pol="normal") where {T<:Union{DG1022Z}} = write(obj,"OUTP$ch:POL $pol")
# Output on/off
